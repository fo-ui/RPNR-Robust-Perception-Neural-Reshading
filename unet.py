...
class UNet(nn.Module):
    ...
    def __init__(self, num_input_channels=3, num_output_channels=3, 
                       ...
                       upsample_mode='deconv', pad='zero', norm_layer=nn.InstanceNorm2d, final_activ = 'sigmoid', need_bias=True, return_deepest = False):
        ...
        self.return_deepest = return_deepest
        ...
        if final_activ == 'sigmoid':
            self.final = nn.Sequential(self.final, nn.Sigmoid())
        elif final_activ == 'tanh':
            self.final = nn.Sequential(self.final, nn.Tanh())
        elif final_activ == 'none':
            self.final = nn.Sequential(self.final)
        elif final_activ == 'relu':
            self.final = nn.Sequential(self.final, nn.ReLU())
        elif 'ClippedRelu' in final_activ:
            if final_activ == 'ClippedRelu':
                v = 1
            else:
                v = int(final_activ.split('_')[1])
            self.final = nn.Sequential(self.final, ClippedRelu(6 / v))
        else:
            self.final = nn.Sequential(self.final)
    ...
    def forward(self, inputs):
        ...
        if not self.return_deepest:
            return self.final(up1)
        else:
            return self.final(up1), deepest
...
def save_unet_dict(unet, path):
    parameter_dict = {
        'num_input_channels': unet.num_input_channels,
        'num_output_channels': unet.num_output_channels,
        'feature_scale': unet.feature_scale,
        'more_layers': unet.more_layers,
        'concat_x': unet.concat_x,
        'upsample_mode': unet.upsample_mode,
        'pad': unet.pad,
        'norm_layer': unet.norm_layer,
        'final_activ': unet.final_activ,
        'need_bias': unet.need_bias,
        'return_deepest': unet.return_deepest,
        'state_dict': unet.state_dict()
    }
    torch.save(parameter_dict, path)

def load_unet_dict(path):
    parameter_dict = torch.load(path)
    state_dict = parameter_dict['state_dict']
    # remove state_dict as key
    del parameter_dict['state_dict']
    unet = UNet(**parameter_dict)
